...
class TestDownloadFile:
    ...
    def test_successful_download(self, tmp_path, mocker):
        ...
        success, result = self.download_file(
            self.session, 'http://test.com/file.pdf', str(test_file))
    ...
    def test_download_fails_on_non_pdf(self, tmp_path, mocker):
        ...
        success, result = self.download_file(
            self.session, 'http://test.com/file.html', str(test_file))
    ...
    def test_download_retries_on_timeout(self, tmp_path, mocker):
        ...
        # 執行下載
        success, result = self.download_file(
            self.session, 'http://test.com/file.pdf', str(test_file), max_retries=3)
    ...
    def test_download_fails_after_max_retries(self, tmp_path, mocker):
        ...
        success, result = self.download_file(
            self.session, 'http://test.com/file.pdf', str(test_file), max_retries=2)
...
class TestGetExamListByYear:
    ...
    def test_retries_on_timeout(self, mocker):
        ...
        exams = self.get_exam_list_by_year(
            self.session, 113, ["警察人員考試"], max_retries=3)
    ...
    def test_returns_empty_after_max_retries(self, mocker):
        ...
        exams = self.get_exam_list_by_year(
            self.session, 113, [], max_retries=2)
