...
class TestGetSaveFolder:
    ...
    @patch('os.remove')
    def test_default_folder(self, mock_remove, mock_open, mock_makedirs, mock_print, mock_input):
        """測試使用預設資料夾"""
    ...
    @patch('builtins.open', create=True)
    @patch('os.remove')
    def test_custom_folder(self, mock_remove, mock_open, mock_makedirs, mock_print, mock_input):
        """測試自訂資料夾"""
        result = self.get_save_folder()
    ...
    @patch('builtins.open', create=True)
    @patch('os.remove')
    def test_permission_error_retry(self, mock_remove, mock_open, mock_makedirs, mock_print, mock_input):
        """測試權限錯誤後重試"""
        result = self.get_save_folder()
