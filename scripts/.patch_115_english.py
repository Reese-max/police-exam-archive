#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""把全 115 年英文黏字、誤併閱讀文章與 Node 契約測試修補加入主修復器。"""
...
node_marker = "AnswerUtils contract loaded before QuizEngine"
if node_marker not in text:
    node_anchor = "\ndef patch_tests_docs_and_ci() -> None:"
    if node_anchor not in text:
        raise SystemExit("cannot locate patch_tests_docs_and_ci insertion point")
    node_override = r'''
...
"""
'''
    text = text.replace(node_anchor, node_override + node_anchor, 1)
