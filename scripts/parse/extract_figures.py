...
TARGETS = [
    ...
    ("交通學系電訊組", "110年", "電路學"),
    ("交通學系電訊組", "111年", "電路學"),
    ("刑事警察學系", "111年", "刑案現場處理與刑事鑑識"),
    ("水上警察學系", "106年", "水上警察情境實務"),
    ("水上警察學系", "107年", "水上警察情境實務"),
    ("消防學系", "107年", "消防警察情境實務"),
    ("鑑識科學學系", "111年", "物理鑑識"),
]
...
def process_target(cat, year, subj_prefix):
    ...
    # 找 PDF（使用舊類科名稱對應 PDF 來源目錄）
    pdf_cat = PDF_CAT_MAP.get(cat, cat)
    pdf_dir = find_subject_dir(PDF_BASE, pdf_cat, year, subj_prefix)
    if not pdf_dir:
        print(f"  ✗ PDF 目錄未找到")
        return 0
